...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        $table->string('f_title', 80)->index()->comment('comment:attribute');
        $table->text('f_description')->comment('comment:attribute');
        $table->timestamps();

        $table->foreign('id_footer')->references('id')->on('vk_titles')->onUpdate('cascade')->onDelete('cascade');
        });
